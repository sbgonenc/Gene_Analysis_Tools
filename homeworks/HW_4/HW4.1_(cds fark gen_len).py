...
def parse_to_Longestpos():
	...
	with open(file) as file_handler:
		for line in file_handler:
			...
			if words[3] == "-": continue
			gene_name = words[12]
			gene_length.append((int(words[7])-int(words[6])))  #cds difference
			diff = int(words[7])-int(words[6])
...
def parse_to_Longestneg():
	...
	with open(file) as file_handler:
		for line in file_handler:
			...
			words = line.strip().split("\t")
			if words[3] == "+": continue
			gene_length.append((int(words[7])-int(words[6])))
			diff = int(words[7]) - int(words[6])  # cds difference
			gene_name = words[12]
			if gene_name not in gene:
				gene[gene_name] = diff
			#elif gene_name in gene and gene[gene_name] < diff:
