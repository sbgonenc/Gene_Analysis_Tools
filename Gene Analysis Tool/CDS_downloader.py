...
class Transcript:
	...
	def tx_length(self):
		...
		for i in range(self.exon_C):
			length += self.exon_E[i] - self.exon_S[i] # +1 #You may wanna change this if you're working with 1-based databases
...
def parse(text, filter=True):
	...
	with open(text) as fh:
		...
		for paragraph, line in enumerate(fh):
			...
			if filter is True:
				#if paragraph > max(statement_42): break
				...
				if cds_end <= cds_start + 12: continue # less than 3 proteins (+stop code)
...
ensembl = parse("Assignments/ensembl_kisa.txt", filter=True)


#with open("Assignments/HW_8/ensembl19_kisa_seq.txt", "w") as fh:
#	fh.write("Tx_id\tcds_length\tcoding_sequence\ttranslation\n")
#
#
def seq_writer(sira):
	'''Gathers sequence from togows, using Transcript class'''
	with open("Assignments/HW_8/ensembl19_kisa_seq.txt", "a") as fh:
		fh.write(f"{sira.tx_id}\t{sira.cds_length2()}\t{sira.cds_sequence()}\t{sira.protein_seq()}\n")


with concurrent.futures.ThreadPoolExecutor() as executor:
	tx_list = [ensembl[e] for e in ensembl]
	executor.map(seq_writer, tx_list)


def output(name, text, filter):
	ensembl_dict = parse(text, filter)

	if name == "tx_length":
		for e in ensembl_dict:
			yield ensembl_dict[e].tx_length()

	if name == "cds_length":
		for e in ensembl_dict:
			yield ensembl_dict[e].cds_length2()
